...
extern "C" {
    ...
    pub fn CDC_Transmit_FS(buf: *mut u8, len: u16) -> u32;
    /* delay for some seconds */
    pub fn HAL_Delay(delay: u32);
    /* the buffer for received data */
    pub static mut cdc_recv_data: [u8; 512];
    pub static mut cdc_recv_len: u32;
}
...
pub fn cdc_send(buffer: &mut [u8], length: usize) -> bool {
    unsafe {
        CDC_Transmit_FS(&mut buffer[0] as *mut u8, length as u16) == 0
    }
}
